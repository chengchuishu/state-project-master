<template>
    <div class="state-machine box" ref="state-machine">
            ...
            <div class="work-bench" ref="work-bench">
                ...
                <div class="designer-canvas" ref="designer-canvas">
                    ...
                    <div class="mouse-tool-wrap" elementtype="mouse-tool-wrap" v-if="mouseToolShow" :style="{top: mouseToolConfig.top+'px', left: mouseToolConfig.left+'px'}">
                        <ul>
                            <li v-if="item.show" v-for="item in mouseTools"><img :src="item.image" /><span>{{item.name}}</span></li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="tool-box" v-show="false">
            <h3>组件</h3>
        ...
        <div class="tool-box data-map"  v-show="false">
            <h3>数据集</h3>
            <div class="tool-content">
                <el-tag
                  :key="tag"
...
export default {
    ...
    methods: {
        ...
        creatGeometryOnSceneByData(data){
            console.log('creatGeometryOnSceneByData :', data)
            let div = document.createElement("div");
            ...
            console.log('name:', data.state.name)
            this.curStateList.push(state)
            div.setAttribute("id", data.state.name + "_div");
            p.setAttribute("id", data.state.name + "_p");
            if(state && state.default === true ) {
              if (data.type !== 'diamond') {
                div.style.background = "yellow"
              } else {
                updateGeometry(canvas, data.type, data.width*this.scale, data.height*this.scale, 'yellow')
              }
              p.innerHTML = `<span><strong>${data.state.name}</strong></span>`;
            }
            ...
            switch (data.type){
                case "fillet_rectangle":
                    ...
                    break;
                case "diamond":
                    //updateGeometry(canvas, data.type, data.width*this.scale, data.height*this.scale, 'red')
        ...
        buildConnectLineByData(data){
            ...
            div.setAttribute("id", data.state.name + "_div");
            p.setAttribute("id", data.state.name + "_p");
            this.curEventList.push(data.state.name)
      ...
      // add  
      updateStateDiv (state) {
        console.log('====state: ', state)
        this.resetAllStateDiv()
        var focusDiv = document.getElementById(state + '_div');
        if(focusDiv) {
            focusDiv.style.backgroundColor = 'green';
            // focusDiv.style.fontWeight = 900;
            // focusDiv.style.color = 'red';
            var focusP = document.getElementById(state + '_p');
            focusP.innerHTML = `<span><strong>${state}</strong></span>`;
        }
      },
      // add  
      updateEventDiv4Arrow (eventName, colorName = 'red') {
        console.log('====eventName: ', eventName)
        console.log('====colorName: ', colorName)
        this.resetAllEventDiv()
        var focusDiv = document.getElementById(eventName + '_div');
        if(focusDiv) {
            // focusDiv.style.backgroundColor = 'green';
            // focusDiv.style.fontWeight = 900;
            // focusDiv.style.color = 'red';
            var focusP = document.getElementById(eventName + '_p');
            focusP.style.color = colorName;
            focusP.innerHTML = `<span><strong>${eventName}</strong></span>`;
        }
      },
      resetAllStateDiv () {
        for (let n = 0; n <this.curStateList.length; n++) {
            var focusDiv = document.getElementById(this.curStateList[n].name + '_div');
            if(focusDiv) {
                if(this.curStateList[n].default){
                    focusDiv.style.backgroundColor = 'yellow';
                } else {
                    focusDiv.style.backgroundColor = 'white';
                }
                // focusDiv.style.fontWeight = 900;
                // focusDiv.style.color = 'red';
                var focusP = document.getElementById(this.curStateList[n].name + '_p');
                focusP.innerHTML = `<span>${this.curStateList[n].name}</span>`;
            }    
        }
      },
      resetAllEventDiv () {
        console.log(this.curEventList)
        for (let n = 0; n <this.curEventList.length; n++) {
            var focusDiv = document.getElementById(this.curEventList[n] + '_div');
            if(focusDiv) {
                var focusP = document.getElementById(this.curEventList[n] + '_p');
                focusP.style.color = 'black';
                focusP.innerHTML = `<span>${this.curEventList[n]}</span>`;
            }    
        }  
      }
    },
